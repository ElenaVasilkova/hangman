# Игра "Виселица"

"Виселица" - консольная игра. Игрок должен угадать загаданное слово, 
предлагая буквы или слово целиком и допустить не более 6 ошибок.

## Особенности реализации
- Угадывание слов русского языка
- Визуальное отображение виселицы
- Отслеживание использованных букв и слов
- Возможность повторной игры
- Автоматический резервный список слов
- Полная валидация ввода
- Модульная архитектура
- Возможность использования подсказки


## Установка и запуск

### Требования
Python 3.6 или выше

### Клонирование удаленного репозитория

```bash
git clone git@github.com:ElenaVasilkova/hangman.git
cd hangman/
```
### Создание виртуального окружения и установка зависимостей
```bash
python3 -m venv venv
source venv/bin/activate
python3 -m pip install --upgrade pip
pip install -r requirements.txt
```

### Запуск игры
```bash
python hangman.py
```

## Структура проекта

```text
hangman/
├── main.py              # Точка входа в приложение
├── game.py              # Основной игровой цикл
├── validators.py        # Валидация ввода пользователя
├── display.py           # Отрисовка виселицы и интерфейса
├── constants.py         # Все текстовые сообщения и настройки
├── words.txt            # Файл со словами
├── README.md            # Документация
└── requirements.txt     # Зависимости проекта
```

## Правила игры

- Компьютер загадывает случайное слово
- Игрок пытается угадать слово, вводя буквы или слова целиком
- При неверной букве/слове теряется попытка и рисуется виселица
- Игра продолжается до угадывания слова или исчерпания 6 попыток
- Если остаётся 3 попытки или менее, игрок может воспользоваться  
  подсказкой - открыть первую букву

## Система подсказок

Когда у игрока остается 3 или меньше попыток, игра предлагает помощь:
```text
Количество оставшихся попыток: 3
*****
Открыть первую букву? (да / нет)
да
Используем подсказку!
Буква С есть в этом слове!
С****
```

## Формат файла words.txt

- Одно слово на строку  
- Только русские буквы  
- В формате UTF-8

Пример:

```text
яблоко
компьютер
программирование
телефон
```

## Архитектура проекта

### Модули и их ответственность

**main.py** - запуск приложения
**game.py** - основной игровой цикл, логика игры
**validators.py** - валидация ввода (буквы, слова, ответы)
**display.py** - отрисовка виселицы
**constants.py** - хранение всех текстов и настроек

### Основные функции

`get_word()` - получение случайного слова (words.py)    
`validate_letter()` - проверка введенной буквы (validators.py)  
`validate_word()` - проверка введенного слова (validators.py)  
`display_word_progress()` - отображение прогресса угадывания (game.py)  
`draw_gallows()` - отрисовка виселицы (display.py)  
`run_game()` - основной игровой цикл (game.py)  
`give_hint()` - система подсказок (game.py)

### Валидация ввода

- Проверка на русский алфавит
- Проверка повторного использования букв/слов
- Валидация ответов "да/нет"
- Обработка пустого ввода

### Пример игровой сессии
```
Давай поиграем в "Виселицу"? (да / нет)
да

У тебя 6 попыток.

*****
Введи одну букву или слово целиком.
а
Буква А есть в этом слове!
```

## Отладка

Если игра не находит файл words.txt, она автоматически использует встроенный список слов.

## Разработчик 
![GitHub](https://img.shields.io/badge/-181717?style=flat-square&logo=github&logoColor=white) [Василькова Елена](https://github.com/ElenaVasilkova)  

![ ](https://img.shields.io/badge/-2CA5E0?style=flat-square&logo=telegram&logoColor=white) [@Vasilkova_Elena_A](https://t.me/Vasilkova_Elena_A)

Язык: Python 3

Лицензия: MIT

## Ревью:
![ ](https://img.shields.io/badge/-2CA5E0?style=flat-square&logo=telegram&logoColor=white) [@grandpraline](https://t.me/zhukovsd_it_chat/53244/270754)
 
## Лицензия

Проект распространяется под лицензией MIT. Подробнее см. в файле [LICENSE](https://github.com/ElenaVasilkova/hangman/blob/main/LICENSE).

---
Приятной игры!
